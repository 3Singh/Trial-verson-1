...
class InjuryPreventionApp:
    def __init__(self, root):
        ...
        self.top_frame = tk.Frame(root, width=840, height=300)
        self.bottom_frame1 = tk.Frame(root, width=840, height=300)
        self.bottom_frame2 = tk.Frame(root, width=840, height=300)

        self.top_frame.grid_rowconfigure((0, 1, 2, 3, 4, 5, 6), weight=1)
    ...
    def create_widgets(self):
        ...
        dropdown_label = tk.Label(self.top_frame, text="Health problem:")
        dropdown_label.grid(row=6, column=0, padx=10, pady=5)

        # Entry fields for user information
        first_name_label = tk.Label(self.top_frame, text="First Name:")
        first_name_label.grid(row=1, column=0, padx=10, pady=5)
        self.first_name_entry = tk.Entry(self.top_frame, font=('Helvetica', 12))
        self.first_name_entry.grid(row=1, column=1, padx=10, pady=5)

        last_name_label = tk.Label(self.top_frame, text="Last Name:")
        last_name_label.grid(row=2, column=0, padx=10, pady=5)
        self.last_name_entry = tk.Entry(self.top_frame, font=('Helvetica', 12))
        self.last_name_entry.grid(row=2, column=1, padx=10, pady=5)

        age_label = tk.Label(self.top_frame, text="Age:")
        age_label.grid(row=3, column=0, padx=10, pady=5)
        self.age_entry = tk.Entry(self.top_frame, font=('Helvetica', 12))
        self.age_entry.grid(row=3, column=1, padx=10, pady=5)

        height_label = tk.Label(self.top_frame, text="Height (cm):")
        height_label.grid(row=4, column=0, padx=10, pady=5)
        self.height_entry = tk.Entry(self.top_frame, font=('Helvetica', 12))
        self.height_entry.grid(row=4, column=1, padx=10, pady=5)

        weight_label = tk.Label(self.top_frame, text="Weight (kg):")
        weight_label.grid(row=5, column=0, padx=10, pady=5)
        self.weight_entry = tk.Entry(self.top_frame, font=('Helvetica', 12))
        self.weight_entry.grid(row=5, column=1, padx=10, pady=5)

        email_label = tk.Label(self.top_frame, text="Email:")
        email_label.grid(row=7, column=0, padx=10, pady=5)
        self.email_entry = tk.Entry(self.top_frame, font=('Helvetica', 12))
        self.email_entry.grid(row=7, column=1, padx=10, pady=5)

        # Submit button to display user information
        submit_button = tk.Button(self.top_frame, text="Submit", command=self.submit_info)
        submit_button.grid(row=8, column=0, columnspan=2, padx=10, pady=10)

        # Text area to display information
        text_frame = ttk.Frame(self.bottom_frame2, style="TFrame")
        text_frame.grid(row=1, column=0, padx=10, pady=10)
        self.text = tk.Text(text_frame, wrap=tk.WORD, width=80, height=10, padx=10, pady=10, font=('Helvetica', 12))
        self.text.pack()

        # Font and sizing
        self.text.tag_configure("title", font=("Helvetica", 14, "bold"))
        self.text.tag_configure("list", font=("Helvetica", 12, "bold"))

        show_more = tk.Button(self.root, text="Show More", command=self.show_more)
        show_more.grid(row=2, column=0, columnspan=2, padx=10, pady=10)
    ...
    def submit_info(self):
        ...
        def show_medical(self):
            ...
            existing_data = read_json_from_file(self,file_path)
            text = tk.Text(self.bottom_frame2, wrap=tk.WORD, width=20, height=10, padx=5, pady=5, font=('Helvetica', 12))
        ...
        medical_history = tk.Button(root, text="Medical History", command=lambda:show_medical(self)) # stops command frmo automatically executing due to the self variable
        medical_history.grid(row=2, column=1, columnspan=2, padx=10, pady=10)
