...
root = Tk()
root.title("Injury Prevention")
root.geometry("840x500")

root.grid_rowconfigure((0,1), weight=1)
root.grid_columnconfigure(0, weight=1)
top_frame = Frame(root, width=840, height=300)
bottom_frame1 = Frame(root, width=840, height=300)
bottom_frame2 = Frame(root, width=840, height=300)

top_frame.grid_rowconfigure((0,1,2,3,4,5,6), weight=1)
top_frame.grid_columnconfigure((0,1), weight=1)

bottom_frame2.grid_rowconfigure((0,1,2), weight=1)
bottom_frame2.grid_columnconfigure((0,1), weight=1)

top_frame.grid(row=0, column=0, sticky = "ew")
bottom_frame1.grid(row=1, column=0, sticky = "ew")
bottom_frame2.grid(row=1, column=0, sticky = "ew")
...
def submit_info():
    ...
    text.insert(tk.END, f"Email Adress: {email_address}\n")
    bottom_frame2.tkraise()
...
def showMore():
    ...
    text.insert(tk.END, f"Height: {height}\n")
    text.insert(tk.END, f"Weight: {weight}\n")
    text.insert(tk.END, f"Email: {email_address}\n")
...
dropdown = Combobox(top_frame, textvariable=dropdown_var, values=problems, font=('Helvetica', 14))
...
first_name_label = Label(top_frame, text="First Name:")
first_name_label.grid(row=1, column=0, padx=10, pady=5)
first_name_entry = Entry(top_frame, font=('Helvetica', 12))
first_name_entry.grid(row=1, column=1, padx=10, pady=5)

last_name_label = Label(top_frame, text="Last Name:")
last_name_label.grid(row=2, column=0, padx=10, pady=5)
last_name_entry = Entry(top_frame, font=('Helvetica', 12))
last_name_entry.grid(row=2, column=1, padx=10, pady=5)

age_label = Label(top_frame, text="Age:")
age_label.grid(row=3, column=0, padx=10, pady=5)
age_entry = Entry(top_frame, font=('Helvetica', 12))
age_entry.grid(row=3, column=1, padx=10, pady=5)

height_label = Label(top_frame, text="Height (cm):")
height_label.grid(row=4, column=0, padx=10, pady=5)
height_entry = Entry(top_frame, font=('Helvetica', 12))
height_entry.grid(row=4, column=1, padx=10, pady=5)

weight_label = Label(top_frame, text="Weight (kg):")
weight_label.grid(row=5, column=0, padx=10, pady=5)
weight_entry = Entry(top_frame, font=('Helvetica', 12))
weight_entry.grid(row=5, column=1, padx=10, pady=5)

email_label = Label(top_frame, text="Email:")
email_label.grid(row=6, column=0, padx=10, pady=5)
email_entry = Entry(top_frame, font=('Helvetica', 12))
email_entry.grid(row=6, column=1, padx=10, pady=5)

text_frame = Frame(top_frame)
text_frame.grid(row=6, column=0, columnspan=2, padx=10, pady=10)

# Submit button to display user information
submit_button = Button(top_frame, text="Submit", command=submit_info)
submit_button.grid(row=7, column=0, columnspan=2, padx=10, pady=10)

text_frame = ttk.Frame(bottom_frame2, style="TFrame")
text_frame.grid(row=1, column=0, columnspan=2, padx=10, pady=10)

# Text area to display information inside the frame
text = tk.Text(text_frame, wrap=tk.WORD, width=60, height=10, padx=10, pady=10, font=('Helvetica', 12))
text.pack()

# Text area to display information
text = tk.Text(bottom_frame2, wrap=tk.WORD, width=60, height=10, padx=10, pady=10, font=('Helvetica', 12))
...
show_more.grid(row=1, column=4, columnspan=2, padx=10, pady=10)


bottom_frame1.tkraise()
root.mainloop()
